...
#ifndef GRAPHWALKER_NODE2VEC_HPP
#define GRAPHWALKER_NODE2VEC_HPP

#include "WalkManager.hpp"
#include "SecondOrderRW.hpp"
#include "IO/VertexIO.hpp"

class Node2Vec : public SecondOrderRW {
private:
    double p;
    double q;
...
public:
    ...
    Node2Vec(bid_t nBlocks, vid_t *startVertex, hid_t walkLength, double p, double q, int begPosFile,
             int csrFile, eid_t maxOutDegree, vid_t nTotalVertices);
...
//        walkManager.moveWalk(walk, threadId, storeBlock);
    ...
    void updateWalk(metrics &m, WalkDataType &walk, bid_t staticBlock, bid_t dynamicBlock, WalkManager &walkManager,
        ...
        if (curVertexInfo.vertexId == 548142 && nextVertex == 77859511){
            int a = 1;
        }
        curVertexInfo.resetVertex(nextVertex, getBlock(nextVertex));
...
Node2Vec::Node2Vec(bid_t nBlocks, vid_t *startVertex, hid_t walkLength, double p, double q, int begPosFile,
                   int csrFile, eid_t maxOutDegree, vid_t nTotalVertices)
...
#if (PLAIN | PLAIN_BUCKET)
void
Node2Vec::updateWalk(metrics &m, const WalkDataType &walk, bid_t execBlock, eid_t *beg_pos, vid_t *csr,
...
double Node2Vec::biasedWeight(VertexInfo &preV, VertexInfo &forwardV, metrics &m) {
...
bool Node2Vec::hasEdge(VertexInfo &srcVertex, vid_t destVertex, metrics &m) {
    if (srcVertex.vertexId == 548142){
        int a = 0;
    }
    if (!srcVertex.csr){
...
#if (PLAIN | PLAIN_BUCKET)
vid_t
Node2Vec::sampleDestVertex(metrics &m, vid_t preVertex, vid_t curVertex, eid_t *beg_pos, vid_t *csr, bid_t execBlock) {
...
#if BI_BLOCK
vid_t Node2Vec::sampleDestVertex(metrics &m, VertexInfo &preVertexInfo, VertexInfo &curVertexInfo) {
    return 0;
}
#endif


#endif //GRAPHWALKER_NODE2VEC_HPP
